...
const LatestActivities = () => {
  ...
  const dealIds = audit?.data?.map(a => a?.targetId);
  ...
  const { data: deals, isLoading: isLoadingDeals } = useList({
    ...
    filters: [{ field: 'id', value: dealIds, operator: 'in' }],
    meta: {
    ...
    >
      {isLoading ? (
        ...
        <List
          ...
          renderItem={i => {
            const deal =
              deals?.data.find(d => d.id === String(i.targetId)) || undefined;
            ...
            return (
              <List.Item>
                <List.Item.Meta
                  ...
                  description={
                    <Space size={4}>
                      <Text strong>{i.user?.name}</Text>
                      <Text>{i.action === 'CREATE' ? 'created' : 'moved'}</Text>
                      <Text strong>{deal?.title}</Text>
                      <Text>deal</Text>
                      <Text>{i.action === 'CREATE' ? 'in' : 'to'}</Text>
